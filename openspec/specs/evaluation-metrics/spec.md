...
Модуль метрик SHALL вычислять распределение классов (цифр 0-9) среди NN реальных картинок для сгенерированных сэмплов (используя метки тестового MNIST), и логировать это распределение (гистограмма по 10 классам). Если модель коллапсирует на несколько классов — гистограмма это покажет.
