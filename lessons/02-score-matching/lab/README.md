...
40 NFE — призрачные контуры, 100 — мягкие читаемые цифры, 400 — чётче и
контрастнее (обобщённый PC: свободное число уровней `m_levels` поверх
непрерывной обёртки, корректор лёгкий — 1 шаг). Важно: тяжёлый корректор
(10 шагов) делает только хуже — Ланжевен-шум усиливает ошибку ε-модели в
зерно, поэтому ранние пресеты (600/1400/3000 с corr=10) выглядели хуже
лёгких. Против 40 NFE у DDIM на VP остаётся разрыв, но уже не 75×:
качественные цифры VE ≈ за 100–400 NFE. Мораль: **непрерывные** SDE/ODE-солверы
на этой модели дают только шум — корректор обязателен; и **внутренние метрики
(eps-MSE) на spiky-сэмплах врут** (строки pc_langevin в CSV хуже шума) —
смотрите на картинки, а не на эту колонку.
