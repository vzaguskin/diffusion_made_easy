...
Конфигурации на каждый бюджет отобраны «внешним судьёй» — eps-MSE сэмплов
под **VP-моделью** (калибровка: реальные данные 0.03, чистый шум 0.67).
Итог противоинтуитивен: **больше NFE — хуже** (judge 0.13 при 40 → ~0.18
при 400–800, плато): Ланжевен-шум корректора за длинной цепочкой
накапливает ошибку грубой ε-модели в зерно. Мягкие читаемые цифры — только
в районе 100 NFE; 40 — призраки; 400/600/800 — всё более плотные шумовые
облака без цифр, сколько ни увеличивай бюджет и как ни дели его между
предиктором и корректором. Тяжёлый корректор (corr=10 у ранних пресетов) —
тоже хуже. Мораль: непрерывные SDE/ODE-солверы на этой модели дают чистый
шум (корректор обязателен), а потолок качества ставит **модель**, не солвер —
лечится preconditioning'ом/взвешиванием (EDM), не тюнингом сэмплинга. Ещё
мораль: внутренняя метрика (eps-MSE под собственной моделью) на spiky-сэмплах
врёт — строки pc_langevin в CSV хуже шума; доверяйте калиброванному судье
или своим глазам.
