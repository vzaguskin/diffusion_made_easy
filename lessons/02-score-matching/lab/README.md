...
600 NFE — намёки на штрихи, 1400 — половина ячеек узнаваема, 3000 — цифры
в большинстве ячеек. Против 40 NFE у DDIM на VP — **75× разрыв по бюджету**
при одной архитектуре сети. Причина та же, что в разделе про спеклы:
ε-параметризация VE без preconditioning/взвешивания вынуждает Ланжевен-корректор
делать за сеть её работу. Ещё мораль: **внутренние метрики (eps-MSE) на таких
сэмплах врут** (spiky-распределение ломает шкалу) — строки pc_langevin в CSV
выглядят хуже шума; смотрите на картинки, а не на эту колонку.
