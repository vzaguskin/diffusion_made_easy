...
VE в этой таблице нет сознательно: его ε-модель при NFE ≤ 400 даёт шум
(запись в CSV есть) — честному VE-сэмплеру лабы (Euler + Ланжевен-корректор)
нужно ~1400 NFE. Это ещё одна грань «VE дешевле только при preconditioning»
из раздела выше.
