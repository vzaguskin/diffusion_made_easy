...
uv run python scripts/train_edm.py         # EDM-ветка VE: обучение + бенчмарк (~16 мин на GPU)
```
...
зерном. Та же цепочка на бюджетах 40–1000 (`level_stride` посещает каждый
k-й уровень лестницы, старт и шумовой поток те же):

![VE PC budgets](../images/lab2_ve_pc_budgets.png)

40–100 — шум (редкая сетка уровней = грубые прыжки Δσ), **200 — цифры
проявляются**, 400–600 — чистые цифры (пик качества), 800–1000 — к цифрам
добавляется зерно корректора. Две важные оговорки из экспериментов вокруг:
...
`train_edm.py` обучает ту же MiniUNet в EDM-параметризации — theory.md §17:
`D_θ(x,σ) = c_skip(σ)·x + c_out(σ)·F_θ(c_in(σ)·x, σ)` (коэффициенты
нормируют вход/выход на всех σ и дают тождественный denoiser при σ→0),
лосс с весом `λ(σ) = (σ²+σ_d²)/(σσ_d)²` (единичная дисперсия цели на всех
уровнях), σ сэмплируется log-normal, сэмплирование — детерминированный Heun
по PF-ODE без всякого Ланжевен-корректора. Бюджет эпох — тот же, что у
naive ve_long (20), для честного сравнения один-к-одному.

Артефакты `runs/edm/` (прогон на 40 эпох — важная деталь ниже):

* `loss_curve.png` — обучение;
* `per_level_vs_naive.png` — denoiser MSE по σ:

  ![EDM per-level](../images/lab2_edm_per_level.png)

  у наивной ветки U-образный перекос (недообученные большие σ), у EDM кривая
  сбалансирована — обещание λ(σ)-взвешивания на графике. (Подводный камень,
  пойманный в лабе: σ_max=5 требует, чтобы log-normal σ-сэмплирование реально
  покрывало диапазон — дефолтные P_mean=−1.2/P_std=1.2 из статьи оставляли
  σ>0.5 недообученными, и сэмплер стартовал вне обученной зоны; здесь
  P_mean=0/P_std=1.6);

* `budget_strip.png` — сэмплы Heun на бюджетах 39–999 NFE + референс наивной
  ветки (её оптимум 600 NFE):

  ![EDM budget strip](../images/lab2_edm_budget_strip.png)

  **Плато вместо острой вершины**: читаемые мягкие цифры на *всех* бюджетах,
  включая 39 NFE, без корректора и без деградации на больших. Детерминированной
  цепи больше не нужен Ланжевен-шум, который ломал большие бюджеты наивной
  ветки. Нижняя граница («когда цифры *начинаются*»):

  ![EDM tiny NFE](../images/lab2_edm_tiny_nfe.png)

  1 NFE (один гигантский Euler-прыжок по всей лестнице) — серая каша;
  **5 NFE — проступают очертания; 11 NFE — читаемые цифры; дальше плато**.
  Для сравнения: наивной VE-ветке на той же архитектуре нужно было 200–600
  NFE *с* корректором;

* `speckle.csv` — спеклы: EDM 0.061 на всех бюджетах против 0.074 у наивного
  оптимума — c_skip/c_out ограничивают выход сети, фоновое зерно ушло.

Итог по трём симптомам наивной ветки (лосс-перекос / спеклы / вершина
бюджета): все три лечатся на уровне модели, не сэмплера — ровно тезис §17
и статьи Karras et al. 2022. Честная оговорка: EDM-ветке потребовалось 40
эпох против 20 у наивной — сбалансированное обучение всех σ дороже, чем
выучить половину диапазона; это цена, а не противоречие.
