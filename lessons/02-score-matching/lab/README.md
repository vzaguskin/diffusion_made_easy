...
* **Почему VE «проигрывает» при равном бюджете — и почему это не вердикт.**
  Три причины: (1) у VE σ-лестница 5 → 0.01 покрывает ~3 порядка величины —
  задача регрессии объективно шире, чем у VP, где √(1−ᾱ) живёт в [0, 1];
  (2) VP получает ancestral-сэмплер в закрытой форме, а VE обходится грубым
  Euler+Ланжевен-корректором; (3) ε-MSE тут без взвешивания по σ — NCSN
  использует веса λ(σ), и без них большие σ недообучены. Третья ветка
  «VE long» (тот же сид, ×4 бюджета) показывает, что VE стабильно
  улучшается (0.105 → 0.090, цифры проступают), но без λ(σ)-взвешивания так и
  не догоняет VP за разумный бюджет — наглядно видно, почему NCSN/EDM без
  взвешивания не обходятся. Современные
  ...
  Правильный вывод из демо: **«VP дешевле наивного VE, но VE догоняет —
  вопрос взвешивания и бюджета»**, а не «VE не нужен».
